...
.defines(function(){
	...
	EntityPlayer = ig.Entity.extend({
		...
		maxVel: { x: 100, y: 110 },
		friction: {x: 150, y: 225 },
		speed: 200,
		ascend: 450,
		sound: {
			jump: new ig.Sound('media/audio/jump.*'),
			attack: new ig.Sound('media/audio/swordswing.*'),
			die: new ig.Sound('media/audio/die.*')
		},
		soundLifetime: 1,
		attackLifetime: 0.24,
		...
		update: function() {
			...
			if(this.pos.x-ig.game.screen.x+this.size.x < 0) {
				ig.game.gameOver = true;
			}
			if(ig.input.pressed('click') || ig.input.pressed('up') || ig.input.pressed('attack')) {
				if(ig.input.mouse.x < ig.system.width/2 || ig.input.pressed('up')) {
					...
					if(ig.game.player.pos.y > ig.game.screen.y) {
						...
						// }
						if(this.currentAnim != this.anims.jump && this.currentAnim != this.anims.fall) {
				...
				else if(ig.input.mouse.x > ig.system.width/2 || ig.input.pressed('attack')) {
	...
	EntityPlayerDeath = ig.Entity.extend({
		...
		friction: {x: 0, y: 225 },
		type: ig.Entity.TYPE.NONE,
		sound: new ig.Sound('media/audio/die.*'),
		init: function(x, y, settings) {
