...
.defines(function(){
	...
	StartScreen = ig.Game.extend({
		...
		init: function() {
			...
			this.BtnEnclave = this.spawnEntity(EntityButtonEnclave, 195, 9);
			this.BtnBlackmoon = this.spawnEntity(EntityButtonBlackmoon, 150, 6);
			this.BtnMoreGames = this.spawnEntity(EntityButtonMoreGames, 120, ig.system.height-47-5);
			...
			ig.music.add('media/audio/covertheme.*');
			ig.music.volume = 0.5;
			if(ig.Sound.enabled) {
				ig.music.play();
		...
		draw: function() {
			...
			this.name.draw(ig.system.width-220-10,65);
			this.title.draw(ig.system.width-169-20,115);
			this.BtnAudio.draw();
			this.BtnStart.draw();
			this.BtnMoreGames.draw();
		}
	});
	...
	HowToScreen = ig.Game.extend({
		font: new ig.Font('media/treasure-map-deadhand.24px.font.png'),
		bg: new ig.Image('media/img/parallax-sky.png'),
		howTo: new ig.Image('media/img/screen-howtoplay.png'),
		keyboardJump: new ig.Image('media/img/keyboard-jump.png'),
		keyboardAttack: new ig.Image('media/img/keyboard-attack.png'),
		...
		draw: function() {
			this.parent();
			this.bg.draw(0,0);
			this.howTo.draw((ig.system.width-435)/2,(ig.system.height-297)/2);
			if(!ig.ua.mobile) {
				this.keyboardJump.draw(10,ig.system.height-49-10);
				this.keyboardAttack.draw(ig.system.width-81-10,ig.system.height-49-10);
			}
			this.buttonContinue.draw((ig.system.width-179)/2,ig.system.height-58-5);
		}
	});
});