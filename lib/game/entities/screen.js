...
.defines(function(){
	...
	StartScreen = ig.Game.extend({
		...
		init: function() {
			...
			this.BtnEnclave = this.spawnEntity(EntityButtonEnclave, 192, 6);
	...
	HowToScreen = ig.Game.extend({
		...
		goal: new ig.Image('media/img/goal.png'),
		...
		draw: function() {
			this.parent();
			this.bg.draw(0,0);
			this.howTo.draw((ig.system.width-400)/2,15); // 435 x 297, (ig.system.height-220)/2, 322
			if(!ig.ua.mobile) {
				this.keyboardJump.draw(5,5);
				this.keyboardAttack.draw(ig.system.width-81-5,5);
			}
			this.goal.draw(5,ig.system.height-68-5); // 150 x 51
