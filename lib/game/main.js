...
GameScreen = ig.Game.extend({
	...
	gamePausedScreen: new ig.Image('media/img/screen-pause-blank.png'),
	gamePausedText: new ig.Image('media/img/screen-pause-text.png'),
	gameCompletedScreen: new ig.Image('media/img/screen-gamecompleted.png'),
	pauseButtonContinue: new ig.Image('media/img/button-continue.png'),
	pauseText: new ig.Image('media/img/screen-pause-text.png'),
	...
	draw: function() {
		if(this.score >= this.maxScore) {
			...
			// 	this.BtnRetry.draw();
			// }
			this.pauseText.draw((ig.system.width-291)/2, (ig.system.height-104-100)/2);
			this.pauseButtonContinue.draw(ig.system.width-179-10, ig.system.height-58-10);
		}
		else if(this.gameOver) {
			// GAME OVER
...
checkOrientation = function() {
	if(ig.ua.mobile && (window.orientation == 0 || window.orientation == 180)) {
		...
		document.getElementById('portrait').innerHTML = '<img src="media/img/screen-rotate.png" />';
